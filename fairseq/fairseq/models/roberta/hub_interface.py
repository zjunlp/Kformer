...
class RobertaHubInterface(nn.Module):
    ...
    def encode(
        self, sentence: str, *addl_sentences, no_separator=False, not_a_sentence=False, add_begin=False, add_eos=False, truncate=False,
    ) -> torch.LongTensor:
        """
        ...
        if truncate:
            ...
            max_len_for_question = 512 - len(bpe_sentence.split()) - 1
            a_sentence = "<s> " + self.bpe.encode(sentence)
            a_sentence = a_sentence.split()[:max_len_for_question]
            bpe_sentence = " ".join(a_sentence)
            bpe_sentence +=  " </s>"
        else:
            if not_a_sentence:
    ...
    def extract_features(
        self, tokens: torch.LongTensor, return_all_hiddens: bool = False, **kwargs
    ) -> torch.Tensor:
        if tokens.dim() == 1:
        ...
        features, extra = self.model(
            ...
            knowledge=knowledge.to(device=self.device) if knowledge is not None else None,
        )
    ...
    def predict(self, head: str, tokens: torch.LongTensor, return_logits: bool = False, **kwargs):
        features = self.extract_features(tokens.to(device=self.device), **kwargs)
